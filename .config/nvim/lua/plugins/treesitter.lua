    ...
    config = function(_, opts)
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        callback = function(ev)
          ...
          end, vim.tbl_extend('force', keymap_opts, { desc = 'Treesitter: 下一个函数开始' }))
          vim.keymap.set({ 'n', 'x', 'o' }, ']c', function()
            move.goto_next_start('@class.outer', 'textobjects')
          end, vim.tbl_extend('force', keymap_opts, { desc = 'Treesitter: 下一个类开始' }))
          vim.keymap.set({ 'n', 'x', 'o' }, ']s', function()
            move.goto_next_start('@statement.outer', 'textobjects')
          end, vim.tbl_extend('force', keymap_opts, { desc = 'Treesitter: 下一个语句开始' }))

          vim.keymap.set({ 'n', 'x', 'o' }, '[f', function()
            move.goto_previous_start('@function.outer', 'textobjects')
          end, vim.tbl_extend('force', keymap_opts, { desc = 'Treesitter: 上一个函数开始' }))
          vim.keymap.set({ 'n', 'x', 'o' }, '[c', function()
            move.goto_previous_start('@class.outer', 'textobjects')
          end, vim.tbl_extend('force', keymap_opts, { desc = 'Treesitter: 上一个类开始' }))
          vim.keymap.set({ 'n', 'x', 'o' }, '[s', function()
            move.goto_previous_start('@statement.outer', 'textobjects')
          end, vim.tbl_extend('force', keymap_opts, { desc = 'Treesitter: 上一个语句开始' }))
